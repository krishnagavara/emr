...
class Ipdpanel extends CI_Controller {
  public function __construct() {
        ...
        $this->load->model('Ipdcharge_model');
        $this->load->model('Common_model');
      
    }
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Ipdcharge_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Ipdcharge';
      $data['activecls']='Ipdpanel';
      $var_array=array($this->session->userdata('office_id'));
      $data['departments']=$this->Common_model->getdepartment($var_array);
      $content=$this->load->view('master/ipdpanel',$data,true);
    ...
    public function savedata()
	{
		...
		$this->form_validation->set_rules('department_id', 'Department', 'trim|required|min_length[1]|max_length[30]|numeric');
    	$this->form_validation->set_rules('description', 'Description', 'trim');
	    if($this->form_validation->run() == TRUE)
	    {
	    	$name=trim(htmlentities($this->input->post('name')));
	    	$department_id=trim(htmlentities($this->input->post('department_id')));
	    	...
	    	$chk_duplication=$this->Ipdcharge_model->checkingval($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Ipdcharge_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Ipdcharge_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['ipdcharge_id'].'">
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Ipdcharge_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Ipdcharge_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Ipdcharge_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Ipdcharge_model->deletedata($delete_id);
	    		if($getresult)
	    		{

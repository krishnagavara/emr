...
class Insurance_company extends CI_Controller {
  public function __construct() {
    ...
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Insurance_company_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Patient Charge Type';
      $data['activecls']='Insurance_company';
      $content=$this->load->view('master/insurance_company',$data,true);
    ...
    private function fetch_data() 
    {
        ...
        return array(
            ...
            'charge_id'=>trim($this->input->post('charge_id')),
    ...
    public function savedata()
	{
		...
		$this->form_validation->set_rules('charge_id', 'Charges', 'trim|required|numeric|min_length[1]|max_length[1]');
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Insurance_company_model->checkingval($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Insurance_company_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Insurance_company_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
	    		foreach($getdata as $data)
	    		{
	    			...
	    			$ch1=$ch2=$ch3=$ch4=$ch5='';
	    			if($data['charge_id']==1){$ch1='selected';}
	    			if($data['charge_id']==2){$ch2='selected';}
	    			if($data['charge_id']==3){$ch3='selected';}
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['insurance_company_id'].'">
							                           <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="'.$this->security->get_csrf_hash().'"> 
							                                 ...
							                                 <div class="row col-md-12">
							                                     <div class="col-md-12 form-group">
							                                         <label for="lastname">Consulation Charges List: </label>
							                                         <select class="form-control" name="charge_id" id="charge_id">
						                                                <option value="1" '.$ch1.'>All</option>
						                                                <option value="2" '.$ch2.'>OPD</option>
						                                                <option value="3" '.$ch3.'>IPD</option>
	...
	public function updatedata()
	{
		...
		$this->form_validation->set_rules('name', 'Name', 'trim|required|min_length[1]|max_length[30]');
		$this->form_validation->set_rules('charge_id', 'Consulation Charges', 'trim|required|numeric|min_length[1]|max_length[1]');
    	$this->form_validation->set_rules('description', 'Description', 'trim');
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Insurance_company_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Insurance_company_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Insurance_company_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Insurance_company_model->deletedata($delete_id);
	    		if($getresult)
	    		{

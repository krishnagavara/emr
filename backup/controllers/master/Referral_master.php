...
class Referral_master extends CI_Controller {
  public function __construct() {
        ...
        $this->load->model('Referral_master_model');
      
    }
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Referral_master_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Referral_master';
      $data['activecls']='Referral_master';
      $content=$this->load->view('master/referral_master',$data,true);
    ...
    public function savedata()
	{
		...
		$this->form_validation->set_rules('place', 'Place', 'trim|required|min_length[1]|max_length[30]');
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Referral_master_model->checkingval($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Referral_master_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Referral_master_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['referral_master_id'].'">
							                                 ...
							                                 <div class="row col-md-12">
							                                  ...
							                                  <div class="row col-md-12">
							                                   <div class="col-md-12 form-group">
							                                         ...
							                                         <input type="text" name="place" id="place" class="form-control" value="'.$data['place'].'">
							                                    </div>
							                                </div>
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Referral_master_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Referral_master_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Referral_master_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Referral_master_model->deletedata($delete_id);
	    		if($getresult)
	    		{

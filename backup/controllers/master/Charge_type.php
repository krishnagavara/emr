...
class Charge_type extends CI_Controller {
  public function __construct() {
    ...
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Charge_type_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Patient Charge Type';
      $data['activecls']='Charge_type';
      $content=$this->load->view('master/charge_type',$data,true);
    ...
    public function savedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Charge_type_model->checkingval($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Charge_type_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Charge_type_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
	    		foreach($getdata as $data)
	    		{
	    			...
	    			if($data['charge_id']==4){$ch4='selected';}
	    			if($data['charge_id']==5){$ch5='selected';}	
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['charge_type_id'].'">
							                                 ...
							                                 <div class="row col-md-12">
							                                     <div class="col-md-12 form-group">
							                                         ...
							                                         <select class="form-control" name="charge_id" id="charge_id">
						                                                ...
						                                                <option value="1" '.$ch1.'>OPD</option>
						                                                <option value="2" '.$ch2.'>IPD</option>
						                                                <option value="3" '.$ch3.'>Laser</option>
						                                                <option value="4" '.$ch4.'>Injection</option>
						                                                <option value="5" '.$ch5.'>Investigation</option>
						                                            </select>
							                                        </div>
							                                </div>
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Charge_type_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Charge_type_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Charge_type_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Charge_type_model->deletedata($delete_id);
	    		if($getresult)
	    		{

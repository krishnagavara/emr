    ...
    function getparticularsdetails(val)
{
    if($('#patient_registration_id').val()=='')
 {
    ...
    if(val>0 && chargetype_id>0)
    {
     ...
     $.ajax({
            type: "POST",
            url: 'Billing/getparticularsdetails',
            dataType: "json",
...
<td><input type="text" step="any" name="disamt[]" id="disamt_'+row_num+'" class="form-control grid_table" value=""  onKeyUp="calcrow('+row_num+')"   onkeypress="return isFloat(event)" required  autocomplete="off"></td>\n\
<td><input type="text" step="any" name="disper[]" id="disper_'+row_num+'" class="form-control grid_table" value=""  onKeyUp="calcrow('+row_num+')"   onkeypress="return isFloat(event)" required  autocomplete="off"></td>\n\
  <td>\n\
...
function calcnet()
  { 
    ...
    var other_charge=findFloatValue('advancedamount');
